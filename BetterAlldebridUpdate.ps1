...
function Check-ForUpdate {
    param (
        [string]$RepoUrl = "https://raw.githubusercontent.com/Pooueto/Powershell/refs/heads/main/BetterAlldebridFriendAPI.ps1"
    )
    ...
    try {
        ...
        $remoteScriptContent = Invoke-WebRequest -Uri $RepoUrl -UseBasicParsing -ErrorAction Stop | Select-Object -ExpandProperty Content

        # Get the content of the current script
        $currentScriptPath = $MyInvocation.MyCommand.Definition
        $currentScriptContent = Get-Content -Path $currentScriptPath -Raw

        # Compare the content (a simple string comparison)
        if ($remoteScriptContent -ne $currentScriptContent) {
            Write-Host "Update found!" -ForegroundColor Green
            return $true
        } else {
            Write-Host "No update available." -ForegroundColor Yellow
            return $false
        }
    } catch {
...
function Apply-Update {
    ...
    try {
        ...
        Invoke-WebRequest -Uri $RepoUrl -OutFile $currentScriptPath -UseBasicParsing -ErrorAction Stop

        Write-Host "Update applied successfully. Please restart the script." -ForegroundColor Green
        return $true
    } catch {
        Write-Host "Error applying update: $($_.Exception.Message)" -ForegroundColor Red

        # Restore the backup if the update failed
...
# URL of the script in the Git repository
$gitRepoUrl = "https://raw.githubusercontent.com/Pooueto/Powershell/refs/heads/main/BetterAlldebridFriendAPI.ps1"

# Check for updates
if (Check-ForUpdate -RepoUrl $gitRepoUrl) {
    $confirm = Read-Host "An update is available. Do you want to download and apply it now? (Y/N)"
    if ($confirm -eq "Y") {
        if (Apply-Update -RepoUrl $gitRepoUrl) {
            # Exit the current script so the user can restart the updated version
            Exit
        }
    }
}

# Rest of your script's initialization and menu logic goes here
# Initialize-Config
# Show-Menu
