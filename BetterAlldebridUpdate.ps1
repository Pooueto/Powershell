...
function Check-ForUpdate {
    Write-Host "Vérification des mises à jour du script..." -ForegroundColor Cyan

    try {
        # Télécharger le contenu du script depuis GitHub
        $latestScriptContent = Invoke-WebRequest -Uri $githubRawUrl -UseBasicParsing | Select-Object -ExpandProperty Content

        # Extraire la version du script distant
        $latestVersionMatch = $latestScriptContent | Select-String -Pattern '^\$scriptVersion\s*=\s*"(.*)"'
        if ($latestVersionMatch) {
            $latestVersion = $latestVersionMatch.Matches[0].Groups[1].Value
            Write-Host "Version locale: $($scriptVersion)"
            Write-Host "Dernière version disponible sur GitHub: $($latestVersion)"
            ...
            if ([version]$latestVersion -gt [version]$scriptVersion) {
                ...
                $confirmUpdate = Read-Host "Voulez-vous télécharger et installer la mise à jour maintenant? (O/N)"
                if ($confirmUpdate -eq "O") {
                    Write-Host "Téléchargement de la mise à jour..." -ForegroundColor Cyan
                    ...
                    try {
                        ...
                        # Écrire la nouvelle version par-dessus l'ancienne
                        $latestScriptContent | Out-File -Path $scriptPath -Encoding UTF8 -Force

                        Write-Host "Mise à jour terminée avec succès. Le script va redémarrer." -ForegroundColor Green

                        # Redémarrer le script
                        Start-Process -FilePath "powershell.exe" -ArgumentList "-File `"$scriptPath`""
                        Exit # Quitter l'instance actuelle du script

                    } catch {
                        Write-Host "Erreur lors de l'installation de la mise à jour: $_" -ForegroundColor Red
                        Write-Host "L'ancienne version a été sauvegardée ici: $backupPath" -ForegroundColor Yellow
                        Move-Item -Path $backupPath -Destination $scriptPath -Force # Tenter de restaurer la sauvegarde si l'écriture échoue
                    }
                }
            } elseif ([version]$latestVersion -eq [version]$scriptVersion) {
                Write-Host "Le script est déjà à jour." -ForegroundColor Green
            } else {
                 Write-Host "Votre version ($scriptVersion) est plus récente que celle sur GitHub ($latestVersion)." -ForegroundColor Yellow
            }

        } else {
            Write-Host "Impossible d'extraire la version du script distant." -ForegroundColor Red
        }

    } catch {
        Write-Host "Erreur lors de la vérification des mises à jour: $_" -ForegroundColor Red
    }
    Write-Host "" # Ligne vide pour une meilleure lisibilité
}
